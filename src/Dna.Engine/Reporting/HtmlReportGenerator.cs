...
{
    public string Generate(string solutionName, List<ProjectAnalysis> projects, Dictionary<string, List<DetectedRisk>> risksMap, AnalysisSettings settings)
    {
        var allRisks = risksMap.SelectMany(x => x.Value).ToList();

        double totalHours = allRisks.Sum(r => r.Level switch
        {
            Severity.High or Severity.Critical => (double)settings.HoursHigh,
            Severity.Medium => (double)settings.HoursMedium,
            _ => (double)settings.HoursLow
        });

        double hourlyRate = (double)settings.HourlyRate;
        string currencySymbol = (string)settings.CurrencySymbol;
        double estimatedCost = totalHours * hourlyRate;
...
<head>
    ...
    <style>
        ...
        .kpi-value {{ display: block; font-size: 28px; font-weight: bold; color: #2c3e50; margin-bottom: 5px; }}
        .kpi-label {{ color: #7f8c8d; font-size: 11px; text-transform: uppercase; letter-spacing: 1px; }}
...
<body>
    <div class='container'>
        ...
        <div class='dashboard'>
            <div class='kpi-card'>
                ...
                <span class='kpi-label'>Projects Found</span>
            </div>
            <div class='kpi-card'>
            ...
            <div class='kpi-card'>
                <span class='kpi-value'>{currencySymbol}{estimatedCost:N0}</span>
        ...
        <div class='footer'>
            Generated by <strong>DotNet Legacy Risk Analyzer (DORA)</strong> • Basado en {currencySymbol}{hourlyRate}/hr • Chile Office
        </div>
    </div>
</body>
</html>");
    ...
    private string GenerateProjectDetails(List<ProjectAnalysis> projects, Dictionary<string, List<DetectedRisk>> risksMap)
    {
        ...
        foreach (var proj in projects)
        {
            ...
            sb.AppendLine($"<div class='project-title'>{System.Net.WebUtility.HtmlEncode(proj.ProjectName)}</div>");
            sb.AppendLine("<div>");
            ...
            if (projectRisks.Any())
            {
                foreach (var risk in projectRisks)
                {
                    ...
                    // 2. Color específico de la CATEGORÍA 
                    string catColor = risk.Category switch
                    {
                        RiskCategory.Security => "#e74c3c", // Rojo
                        RiskCategory.Architecture => "#3498db", // Azul
                        RiskCategory.Tooling => "#9b59b6", // Morado
                        RiskCategory.Frontend => "#e67e22", // Naranja
                        RiskCategory.Database => "#16a085", // Verde oscuro
                        _ => "#7f8c8d" // Gris por defecto
                    };
                    ...
                    sb.AppendLine($"<span style='background-color:{catColor}; color:white; font-size:10px; padding:3px 6px; border-radius:4px; font-weight:bold; margin-bottom:5px; display:inline-block; text-transform:uppercase; letter-spacing:0.5px;'>{risk.Category}</span>");

                    sb.AppendLine($"<strong>{System.Net.WebUtility.HtmlEncode(risk.Description)}</strong>");
                    sb.AppendLine($"<span class='recommendation'>💡 Suggestion: {System.Net.WebUtility.HtmlEncode(risk.Recommendation)}</span>");
                    sb.AppendLine("</div></div>");
                }
            }
            else
